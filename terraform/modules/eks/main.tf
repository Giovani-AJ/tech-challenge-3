## Cluster EKS + Node Group + provider OIDC (para IRSA: ESO, service accounts dos
## microsserviços que falam com SQS/DynamoDB, etc).
##
## Conta pessoal: as roles de IAM são criadas livremente aqui (sem restrição de LabRole).
...
# Concede admin explicitamente a quem estiver rodando o Terraform. Não dependemos
# do bootstrap automático do "cluster creator" (não é confiável em todo cenário,
# ex: depois de um -replace do cluster) — isso é o que faz o provider kubernetes/
# helm conseguir criar namespace/release logo em seguida, no mesmo apply.
...
# Provider OIDC do cluster, base de tudo que é IRSA (ESO, SQS/DynamoDB access, etc).
